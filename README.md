...
- `pnpm run translations:status` — Show translation status (pass language via `--`, e.g. `pnpm run translations:status -- fr`)
- `pnpm run translations:create` — Create a new locale file (pass language via `--`, e.g. `pnpm run translations:create -- de-AT`)
- `pnpm run translations:migrate` — Migrate renamed translation keys (pass mappings via `--`, e.g. `pnpm run translations:migrate -- old.id=new.id`)
